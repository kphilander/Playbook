---
content_type: jurisdiction-module
title: "Singapore — Compliance Module"
pillar: [open]
tier: 1
tone: [confident-informative]
reading_level: grade-9-12
game_type: []
audience: [general]
channel: [blog, in-app]
cultural_profile:
  voice: peer
  framing: individual
  humor: irreverent
  directness: blunt
  comfort: open
presentation:
  odds_format: decimal
  currency: sgd
  sports_culture: "football, horse racing"
  language: en-sg
verticals: [land-based]
regulatory_approach: prescriptive
adaptation_status: base
adaptation_notes: |
  Singapore is a standalone jurisdiction (city-state) with a single
  national regulatory regime. No sub-jurisdiction modules needed.
  Land-based casino only -- online gambling is prohibited. Cultural
  adaptation needed for multilingual context (English, Mandarin,
  Malay, Tamil). Entry levy and automatic exclusion provisions are
  unique to Singapore and require specific messaging.
last_updated: "2026-03-22"
---

# Singapore — Compliance Module

> **Operator note**: This module covers every compliance requirement for deploying Playbook in Singapore's two integrated resort casinos. Work through each section, complete the compliance checklist at the bottom, and get legal/compliance sign-off before launch. Singapore has a **prescriptive** regulatory approach with detailed requirements and criminal penalties for violations.
>
> **Verticals covered**: Land-based Casino only
> *(Singapore prohibits online gambling under the Gambling Control Act 2022. This module covers land-based casino operations at Marina Bay Sands and Resorts World Sentosa only.)*

> **Last verified**: 2026-03-22
> **Next review due**: 2026-06-22 *(quarterly, per [governance cadence](../../brand-book/08-governance.md))*

---

## Quick-scan index

| Section | Verticals | Description |
|---|---|---|
| [Regulatory authority](#regulatory-authority) | All | Who regulates, key contacts |
| [Legal requirements](#legal-requirements) | All | Licensing, permitted products, legal framework |
| [Helpline](#helpline) | All | NCPG helpline details and display rules |
| [Messaging requirements](#messaging-requirements) | All | Required statements and on-brand integration |
| [Advertising restrictions](#advertising-restrictions) | All | GRA prior-approval regime |
| [Self-exclusion](#self-exclusion) | All | Multi-tier NCPG exclusion system |
| [Player protection -- land-based](#player-protection--land-based) | Land-based | Casino-specific player protection tools |
| [Age verification](#age-verification) | All | 21+ minimum age and verification requirements |
| [AML/KYC](#amlkyc) | All | Anti-money laundering and identity verification |
| [Entry levy](#entry-levy) | All | Unique to Singapore -- citizen/PR entry fees |
| [Collateral adaptation](#collateral-adaptation) | All | What to change across all touchpoints |
| [`_brand.yml` updates](#brandyml-updates) | All | Config values to add |
| [Compliance checklist](#compliance-checklist) | All | Pre-launch verification |

---

## Regulatory authority

| Field | Value |
|---|---|
| **Primary regulator** | Gambling Regulatory Authority (GRA) |
| **Website** | [www.gra.gov.sg](https://www.gra.gov.sg/) |
| **Governing legislation** | Casino Control Act 2006; Gambling Control Act 2022; Gambling Regulatory Authority of Singapore Act 2022 |
| **Key subsidiary legislation** | Casino Control (Responsible Gambling) Regulations 2013; Casino Control (Advertising) Regulations 2010; Casino Control (Prevention of Money Laundering, Terrorism Financing and Proliferation Financing) Regulations 2009 |
| **Reporting to** | Ministry of Home Affairs (MHA) |
| **Regulatory approach** | **Prescriptive** -- detailed regulations with specific requirements, prior-approval regimes, and criminal penalties |
| **Harm minimisation body** | National Council on Problem Gambling (NCPG), under Ministry of Social and Family Development (MSF) |

### Regulatory history

- **2006**: Casino Control Act enacted in preparation for integrated resorts
- **2010**: Marina Bay Sands and Resorts World Sentosa open; advertising and entry levy regulations take effect
- **2013**: Responsible Gambling Regulations enacted
- **2022**: GRA reconstituted from Casino Regulatory Authority (CRA) on 1 August 2022; Gambling Control Act 2022 and GRA Act 2022 enacted, absorbing the former Remote Gambling Act 2014
- **2024**: Casino Control (Amendment) Act 2024 -- criminalises breach of family visit limit and strengthens exclusion enforcement

### Upcoming changes

The Casino Control (Amendment) Act 2024 was gazetted on 3 October 2024. Key changes include criminalising the breach of family visit limits (previously only third-party exclusions were criminal offences) and expanding the scope of automatic exclusion by law. Operators should verify implementation dates with GRA.

**Source**: [Casino Control (Amendment) Act 2024](https://sso.agc.gov.sg/Acts-Supp/29-2024/Published/20241003?DocDate=20241003)

---

## Legal requirements

| Requirement | Value |
|---|---|
| **Minimum gambling age** | 21+ |
| **Legal framework** | Centralised national regulation under GRA |
| **Online gambling** | **Prohibited** -- unlawful under Gambling Control Act 2022; penalties up to S$10,000 fine or 6 months imprisonment for participants; up to S$500,000 or 7 years imprisonment for operators |
| **Permitted products** | Land-based casino gaming at 2 integrated resorts only |

### Permitted products table

| Product | Vertical | Legal status | Regulator | Notes |
|---|---|---|---|---|
| Casino gaming (table games, slots) | Land-based | Legal (licensed) | GRA | Only at Marina Bay Sands and Resorts World Sentosa |
| Online casino / iGaming | Interactive | **Prohibited** | GRA / SPF | Criminal offence under Gambling Control Act 2022 |
| Sports betting (online) | Interactive | **Prohibited** (except Singapore Pools) | GRA | Singapore Pools holds limited exemption |
| Lottery | N/A | Singapore Pools monopoly | GRA | Regulated separately |
| Horse racing | N/A | Singapore Turf Club (ceased in 2024) | GRA | Singapore Turf Club ceased operations in 2024 |

**Source**: [Casino Control Act 2006](https://sso.agc.gov.sg/Act/CCA2006) | [Gambling Control Act 2022](https://sso.agc.gov.sg/Act/GCA2022)

---

## Helpline

| Field | Value |
|---|---|
| **Name** | National Council on Problem Gambling (NCPG) |
| **Phone** | 1800-6-668-668 |
| **Website** | [www.ncpg.org.sg](https://www.ncpg.org.sg/) |
| **Hours** | Daily, 8am -- 11pm |
| **Languages** | English, Mandarin, Malay, Tamil |
| **Cost** | Free (toll-free within Singapore) |
| **Additional services** | Webchat (daily 8am-11pm), e-counselling, face-to-face counselling via The Resilienz Clinic |

### Display rules

#### Land-based display

Casino operators must display NCPG helpline information (1800-6-668-668) prominently in casino premises as part of their GRA-approved responsible gambling programme. This includes gaming floors, entry points, and areas where responsible gambling information is posted.

**Source**: [Casino Control (Responsible Gambling) Regulations 2013](https://sso.agc.gov.sg/SL/CCA2006-S332-2013)

#### Interactive display

Not applicable -- online gambling is prohibited in Singapore.

### On-brand helpline display

**Bare compliance** (what most operators do):
> "If you or someone you know has a gambling problem, call 1800-6-668-668."

**On-brand compliance** (the Playbook way):
> Need to talk? The NCPG helpline is free, confidential, and available daily 8am-11pm: **1800-6-668-668**. Chat online at ncpg.org.sg. No judgement, just support.

*Pattern from: [Messaging Framework -- Warning Statement Standards](../../brand-book/05-messaging-framework.md#warning-statement-standards)*

---

## Messaging requirements

### Messaging regime summary

| Context | Obligation type | What's required | Source |
|---|---|---|---|
| Casino premises | Obligation-based | Display responsible gambling information including helpline and self-exclusion details | CCA Part XI; RG Regs 2013 |
| Advertising/promotions | Standard-based | All casino ads must receive prior GRA approval; no glamorisation; no targeting minors/vulnerable | Advertising Regs 2010 |
| Entry points | Standard-based | Entry levy information, age restriction (21+), NCPG helpline | CCA s.116, s.132 |
| Exclusion/visit limit areas | Obligation-based | Information about self-exclusion, family exclusion, and visit limit schemes | CCA ss.159-168 |

**Obligation types explained:**
- **Verbatim** -- exact prescribed text that must appear word-for-word
- **Obligation-based** -- must post information meeting a described standard, but no prescribed wording
- **Standard-based** -- must display specific items (e.g., helpline name, program name) but surrounding copy is flexible

### Verbatim required statements

Singapore does not prescribe verbatim statements in the same manner as some US jurisdictions. The regime is obligation-based: operators must display responsible gambling information, helpline details, and exclusion programme information as part of their GRA-approved responsible gambling programme. The specific content and format are subject to GRA approval.

### Obligation-based requirements

| Obligation | What must be communicated | Where | Source |
|---|---|---|---|
| Responsible gambling information | Problem gambling signs, available help, self-exclusion options | Casino premises | RG Regs 2013 |
| NCPG helpline | 1800-6-668-668 | Casino premises, advertising | RG Regs 2013 |
| Entry levy | S$150/24hrs or S$3,000/year for citizens/PRs | Casino entry points | CCA s.116 |
| Age restriction | 21+ to enter casino | Casino entry points | CCA s.132 |
| Exclusion programmes | Self-exclusion and family exclusion information | Casino premises | CCA ss.159-168 |

### On-brand integration

Singapore uses obligation-based requirements, so operators have flexibility to use Playbook messaging:

**Generic operator approach:**
> "Gambling can be addictive. Call 1800-6-668-668 for help."

**Playbook on-brand approach:**
> Know your game, know your limits. If gambling stops being fun, the NCPG helpline is free and confidential: **1800-6-668-668** (daily 8am-11pm). You can also chat online at ncpg.org.sg.

**Rules for on-brand integration:**
1. NCPG helpline number (1800-6-668-668) must appear exactly as provided
2. Obligation-based requirements can use full Playbook voice as long as the obligation is met
3. Give all messaging visual dignity -- don't shrink to minimum size
4. Follow with a helpful action whenever possible
5. All messaging subject to GRA approval as part of the responsible gambling programme

*Pattern from: [Messaging Framework -- Warning Statement Standards](../../brand-book/05-messaging-framework.md#warning-statement-standards)*

---

## Advertising restrictions

| Rule | Requirement | Verticals | Source |
|---|---|---|---|
| Prior approval | All casino advertising and promotions require **prior GRA approval** | Land-based | Advertising Regs 2010, Reg 3 |
| No glamorisation | Must not glamorise gambling or encourage irresponsible/excessive gambling | Land-based | Advertising Regs 2010 |
| No targeting minors | Must not target persons under 21 | Land-based | Advertising Regs 2010 |
| No targeting vulnerable | Must not target vulnerable persons | Land-based | Advertising Regs 2010 |
| No misleading claims | Must not contain misleading claims about odds or winnings | Land-based | Advertising Regs 2010 |
| Designated sites only | Limited factual advertisements permitted only at designated sites | Land-based | Advertising Regs 2010, Reg 5 |

### Channel-specific rules

| Channel | Key restrictions | Verticals | Source |
|---|---|---|---|
| Broadcast (TV/radio) | Casino advertising heavily restricted; requires GRA approval | Land-based | Advertising Regs 2010 |
| Digital | Online casino advertising prohibited (online gambling is illegal) | N/A | GCA 2022 |
| Print | Limited factual ads at designated tourist sites, hotels, Changi Airport only | Land-based | Advertising Regs 2010, Reg 5 |
| Direct marketing | Subject to GRA approval | Land-based | Advertising Regs 2010 |
| In-venue | Subject to GRA approval as part of casino operations | Land-based | Advertising Regs 2010 |
| Sponsorship | Subject to GRA approval | Land-based | Advertising Regs 2010 |

### Prohibited content

- Glamorisation of gambling or gambling lifestyle
- Claims or implications that gambling is a means of financial gain
- Targeting of minors (under 21) or vulnerable persons
- Misleading claims about odds, house edge, or winnings
- Encouragement of irresponsible or excessive gambling
- Advertising of unlawful gambling activities (fine up to S$20,000)

### Required disclosures

All approved casino advertisements must comply with GRA guidelines. The specific disclosure requirements are determined as part of the GRA approval process.

### Enforcement

Enforcement is active. Marina Bay Sands was fined SGD100,000 for launching casino-related promotions without prior GRA approval, demonstrating that the prior-approval requirement is strictly enforced.

**Source**: [Casino Control (Advertising) Regulations 2010](https://sso.agc.gov.sg/SL/CCA2006-S86-2010) | [GRA Responsible Gambling, Advertising & Promotion](https://www.gra.gov.sg/harm-minimisation/responsible-gambling-advertising-promotion)

---

## Self-exclusion

Singapore has the most comprehensive exclusion system in Asia-Pacific, administered by the NCPG with multiple tiers:

### Exclusion types overview

| Type | Initiated by | Duration | Scope | Criminal penalty for breach |
|---|---|---|---|---|
| **Self-exclusion** | Individual (voluntary) | Minimum 1 year | All casinos, jackpot rooms, Singapore Pools online | No (self-imposed) |
| **Voluntary visit limit** | Individual (voluntary) | Ongoing | Limits casino visits per month | No (self-imposed) |
| **Family exclusion order** | Family member | As determined by NCPG | All casinos | No |
| **Family visit limit** | Family member | As determined by NCPG | Limits casino visits per month | **Yes** -- up to S$10,000 fine or 12 months imprisonment |
| **Third-party exclusion order** | NCPG (without application) | As determined by NCPG | All casinos | **Yes** -- up to S$10,000 fine or 12 months imprisonment |
| **Exclusion by law** | Automatic | Duration of qualifying condition | All casinos, jackpot rooms, Singapore Pools online | **Yes** |

### Self-exclusion details

| Field | Value |
|---|---|
| **Program name** | NCPG Self-Exclusion |
| **Duration options** | Minimum 1 year; revocation requires application to NCPG |
| **Scope** | All casinos in Singapore, jackpot machine rooms in private clubs, Singapore Pools remote betting |
| **Enrollment method** | Online via NCPG IVEMS portal or in person |
| **Reinstatement** | Application to NCPG after minimum 1 year |
| **Source** | [CCA ss.159-168](https://sso.agc.gov.sg/Act/CCA2006) |

### Exclusion by law (automatic)

The following categories of persons are automatically excluded from casinos, jackpot rooms, and Singapore Pools online betting:

- Undischarged bankrupts
- Recipients of ComCare Long Term (Public Assistance), Medium Term, or Short Term Assistance
- Occupants or tenants in HDB Public Rental Scheme flats
- Recipients of legal aid under the Legal Aid Bureau

This is automatic -- no application or revocation is required. The exclusion is lifted when the qualifying condition ends.

**Source**: [NCPG Overview of Exclusions and Visit Limit](https://www.ncpg.org.sg/services/overview-of-exclusions-and-visit-limit)

### Playbook language mapping

| Context | Playbook term | Official term | When to use official term |
|---|---|---|---|
| Tier 1 (casual) | "Take a break" / "Step away" | Self-Exclusion / Visit Limit | Never in Tier 1 |
| Tier 2 (formal) | "Self-exclusion" | NCPG Self-Exclusion | Legal documents, formal enrollment |
| Staff training | Both | NCPG Self-Exclusion / Family Exclusion Order / Visit Limit | When explaining the specific programme |

### Staff FAQ addition

Add this Q&A to the [Staff FAQ](../../collateral/customer-service/staff-faq.md):

> **Q: What exclusion options are available in Singapore?**
>
> Singapore has the most comprehensive casino exclusion system in Asia-Pacific, administered by the NCPG. Options include: self-exclusion (minimum 1 year from all casinos), voluntary visit limits, family exclusion orders, and family visit limits. Some categories of persons (bankrupts, social assistance recipients) are automatically excluded by law. If a patron asks about any of these, explain their options clearly and offer to help them contact the NCPG at 1800-6-668-668 or visit ncpg.org.sg. Use "take a break" in casual conversation; use the official programme names when explaining specifics.

---

## Player protection -- land-based

### Required tools and obligations

| Tool / Obligation | Required? | Details | Source |
|---|---|---|---|
| Problem gambling information posting | Yes | Prominent display in casino premises | RG Regs 2013 |
| Helpline display | Yes | NCPG 1800-6-668-668 | RG Regs 2013 |
| Expenditure limits | Yes | Systems for patrons to set expenditure limits | RG Regs 2013 |
| Time limits | Yes | Systems for patrons to set time limits | RG Regs 2013 |
| Employee training | Yes | Compulsory for frontline and management staff | RG Regs 2013 |
| Self-exclusion | Yes | Multi-tier NCPG system | CCA ss.159-168 |
| Entry levy | Yes | S$150/24hrs or S$3,000/year for citizens/PRs | CCA s.116 |
| Responsible gambling programme | Yes | Must be GRA-approved; subject to regular inspection and audit | RG Regs 2013 |
| Age verification at entry | Yes | 21+ verification at casino entrance | CCA s.132 |

### Playbook tool messaging -- land-based

| Tool | Playbook copy | Context |
|---|---|---|
| Expenditure limits | "Set a spending limit before you play. It takes 30 seconds and keeps you in control." | RG Regs 2013 |
| Time limits | "Set a time reminder -- your session, your rules." | RG Regs 2013 |
| Self-exclusion | "Need a longer break? Self-exclusion is free, confidential, and covers all casinos in Singapore." | CCA ss.159-168 |
| Helpline | "The NCPG helpline is free and confidential: 1800-6-668-668, daily 8am-11pm." | RG Regs 2013 |

---

## Player protection -- interactive

Not applicable -- online gambling is prohibited in Singapore under the Gambling Control Act 2022.

---

## Age verification

| Field | Value |
|---|---|
| **Minimum age** | 21+ |
| **`_brand.yml` key** | `legal.minimum_gambling_age.singapore` |
| **`{{MIN_AGE}}` token value** | 21 |
| **Statutory basis** | [CCA s.132](https://sso.agc.gov.sg/Act/CCA2006) |

### Land-based verification

| Requirement | Details | Source |
|---|---|---|
| **Method** | Age verification at casino entrance by casino staff | CCA s.132 |
| **Acceptable ID** | Government-issued photo identification | CCA |
| **Enforcement** | Persons under 21 prohibited from entering casino premises | CCA s.132 |

### Interactive verification

Not applicable -- online gambling is prohibited in Singapore.

### Age messaging

All collateral in this jurisdiction must display 21+ age notice:

> "You must be 21+ to enter the casino."

---

## Entry levy

*This section is unique to Singapore.*

| Field | Value |
|---|---|
| **Applies to** | Singapore citizens and permanent residents only |
| **24-hour levy** | S$150 |
| **Annual levy** | S$3,000 |
| **Foreigners** | Exempt |
| **Purpose** | Social safeguard to minimise harmful gambling among locals |
| **Revenue allocation** | Paid to Singapore Totalisator Board for social/charitable purposes |
| **Source** | [CCA s.116](https://sso.agc.gov.sg/Act/CCA2006); [GRA Entry Levy](https://www.gra.gov.sg/resources/entry-levy) |

### Playbook messaging for entry levy

The entry levy is a unique Singapore feature that requires careful messaging:

**Generic approach:**
> "Singapore citizens and permanent residents must pay an entry levy to enter the casino."

**Playbook on-brand approach:**
> Singapore citizens and PRs: casino entry requires a levy of S$150 (24 hours) or S$3,000 (annual). This is a social safeguard designed to encourage informed decisions about casino visits. Foreigners are exempt.

---

## AML/KYC

| Requirement | Value |
|---|---|
| **AML authority** | GRA (under Casino Control Act); MAS (financial institutions) |
| **KYC/CDD threshold** | S$4,000 for casino transactions |
| **Governing regulation** | Casino Control (Prevention of Money Laundering, Terrorism Financing and Proliferation Financing) Regulations 2009 |
| **Reporting requirements** | Suspicious transaction reports (STRs) to Suspicious Transaction Reporting Office (STRO) |
| **Player-facing impact** | Identity verification for transactions at or above S$4,000; source of funds checks for large transactions |

### Player-facing messaging

When AML/KYC processes affect players, use Playbook voice:

**Bare compliance:**
> "We are required by law to verify your identity."

**On-brand:**
> "Quick ID check -- it keeps your account secure and is required for all players. Standard procedure, takes just a moment."

---

## Collateral adaptation

Quick-reference table mapping every collateral category to Singapore-specific adaptations.

| Category | Collateral | Verticals | Adaptation required | Token |
|---|---|---|---|---|
| **Environmental** | Venue signage | Land-based | Add NCPG helpline, 21+ age notice, entry levy info; consider multilingual (EN/ZH/MS/TA) | `{{HELPLINE_SG}}`, `{{MIN_AGE}}` |
| **Environmental** | Digital display | Land-based | NCPG helpline, entry levy, exclusion info | `{{HELPLINE_SG}}` |
| **Print** | Brochure | Land-based | Add NCPG helpline, entry levy info, exclusion programme details | `{{HELPLINE_SG}}` |
| **Print** | Rack card | Land-based | NCPG helpline, self-exclusion info | `{{HELPLINE_SG}}` |
| **Print** | Table tent | Land-based | NCPG helpline, expenditure/time limit info | `{{HELPLINE_SG}}` |
| **Print** | Helpline card | Land-based | Replace helpline with NCPG 1800-6-668-668 | `{{HELPLINE_SG}}` |
| **Customer service** | Conversation scripts | Land-based | Add entry levy FAQ, exclusion by law explanation, NCPG referral process | N/A |
| **Customer service** | Staff FAQ | Land-based | Add Singapore-specific Q&As (entry levy, exclusion types, NCPG) | N/A |

**Note**: Digital, email, social media, video/audio, and online collateral categories are not applicable for Singapore deployment as online gambling is prohibited. All adaptations are for land-based casino venues only.

For a detailed collateral adaptation guide, see the [collateral adaptation template](../_template/collateral-adaptation.md).

---

## `_brand.yml` updates

Add these entries to your `_brand.yml`:

```yaml
# --- HELPLINES ---------------------------------
helplines:
  singapore:
    national:
      number: "1800-6-668-668"
      website: "https://www.ncpg.org.sg"
      label: "NCPG (National Council on Problem Gambling)"
      hours: "Daily 8am-11pm"
      languages: "English, Mandarin, Malay, Tamil"

# --- LEGAL -------------------------------------
legal:
  minimum_gambling_age:
    singapore: 21
  entry_levy:
    singapore_24hr: 150
    singapore_annual: 3000
    singapore_currency: "SGD"

# --- MESSAGING ---------------------------------
messaging:
  mandatory:
    singapore-general: "If you or someone you know has a gambling problem, call 1800-6-668-668."
    # Obligation-based -- no prescribed wording; above is suggested minimum
```

---

## Compliance checklist

Complete before launching Playbook in Singapore.

### Regulatory
- [ ] Identified primary regulator: GRA (under Ministry of Home Affairs)
- [ ] Confirmed Casino Control Act 2006, Gambling Control Act 2022, and all subsidiary regulations
- [ ] Verified legal gambling age: 21+
- [ ] Reviewed permitted products: land-based casino only (2 integrated resorts)
- [ ] Checked for pending regulatory changes (Casino Control (Amendment) Act 2024)
- [ ] Identified deployment vertical: Land-based only

### Messaging
- [ ] Obligation-based requirements met with on-brand messaging
- [ ] NCPG helpline (1800-6-668-668) displayed per GRA-approved RG programme
- [ ] Age notice (21+) on all player-facing content
- [ ] Entry levy information displayed at casino entry points
- [ ] All `{{PLACEHOLDER}}` tokens resolve to Singapore values
- [ ] Multilingual considerations addressed (EN/ZH/MS/TA)

### Advertising
- [ ] All advertising/promotional content submitted for **prior GRA approval**
- [ ] No glamorisation of gambling
- [ ] No targeting of minors (under 21) or vulnerable persons
- [ ] No misleading claims about odds or winnings
- [ ] Limited factual ads distributed only at designated sites
- [ ] No advertising of online gambling (prohibited)

### Self-exclusion / Exclusion
- [ ] Self-exclusion information and NCPG referral process in place
- [ ] Family exclusion order information available
- [ ] Visit limit scheme information available
- [ ] Staff trained on all exclusion types (self, family, third-party, by law)
- [ ] Exclusion by law categories understood (bankrupts, social assistance, public rental, legal aid)
- [ ] Self-excluded patrons removed from all marketing

### Player protection -- land-based
- [ ] GRA-approved responsible gambling programme implemented
- [ ] Problem gambling information posted per programme requirements
- [ ] NCPG helpline displayed in required locations
- [ ] Employee training programme implemented (frontline and management)
- [ ] Expenditure limit system available for patrons
- [ ] Time limit system available for patrons
- [ ] Entry levy system functioning (S$150/24hrs, S$3,000/year for citizens/PRs)

### Age verification
- [ ] Age verification at casino entrance (21+)
- [ ] Government-issued photo ID required

### AML/KYC
- [ ] Customer Due Diligence at S$4,000 threshold
- [ ] Suspicious transaction reporting to STRO
- [ ] Records maintained per regulatory requirements

### Content
- [ ] All collateral adapted per collateral adaptation table
- [ ] Staff FAQ updated with Singapore-specific Q&As
- [ ] Conversation scripts updated with entry levy and exclusion information
- [ ] Multilingual needs assessed and addressed

### Governance
- [ ] `_brand.yml` updated with Singapore values
- [ ] `Last verified` date set
- [ ] `Next review due` date set (quarterly)
- [ ] Legal/compliance sign-off obtained
- [ ] Brand owner sign-off obtained
- [ ] GRA approval obtained for responsible gambling programme

---

*Cross-references: [`_brand.yml`](../../_brand.yml) | [Messaging Framework -- Warning Statement Standards](../../brand-book/05-messaging-framework.md#warning-statement-standards) | [Staff FAQ](../../collateral/customer-service/staff-faq.md) | [Governance](../../brand-book/08-governance.md) | [Jurisdictions README](../README.md)*
