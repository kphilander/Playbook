---
content_type: jurisdiction-module
title: "Illinois — Advertising Rules"
pillar: [open]
tier: 1
tone: [confident-informative]
reading_level: grade-9-12
game_type: [slots, blackjack, roulette, sports-betting, video-poker]
audience: [general]
channel: [blog, in-app]
cultural_profile:
  voice: peer
  framing: individual
  humor: irreverent
  directness: blunt
  comfort: open
presentation:
  odds_format: american
  currency: usd
  sports_culture: us-major-leagues
  language: en-us
adaptation_status: base
adaptation_notes: |
  Advertising rules are jurisdiction-specific regulatory content. Illinois
  harmonized its advertising rules across casinos, video gaming, and sports
  wagering effective August 1, 2025, building on 11 Ill. Adm. Code 1900.340.
  On-brand disclosure examples need cultural adaptation for non-US markets.
last_updated: 2026-06-05
---

# Illinois — Advertising Rules

> **Operator note**: This is the detailed advertising reference for Illinois. It covers the sports wagering advertising standard (11 Ill. Adm. Code 1900.340), the August 2025 uniform rules that extended consistent requirements across casinos, video gaming, and sports wagering, channel-by-channel analysis, prohibited content, required disclosures, and a {{PROGRAM_NAME}} content compliance matrix. Use this alongside the [Illinois compliance module](README.md) for complete coverage.

> **Last verified**: 2026-06-05

---

## Quick-scan index

| Section | Description |
|---|---|
| [Illinois's prescriptive standard](#illinoiss-prescriptive-standard) | 1900.340 + Aug 2025 uniform rules |
| [What changed in August 2025](#what-changed-in-august-2025) | Uniform rules across all verticals |
| [AGA Responsible Gaming Code](#aga-responsible-gaming-code) | Industry self-regulatory overlay |
| [Channel-by-channel rules](#channel-by-channel-rules) | Broadcast, digital, print, direct, in-venue |
| [Prohibited content](#prohibited-content) | What you cannot say, show, or where you cannot place |
| [Required disclosures](#required-disclosures) | What must appear in advertising |
| [Direct advertising requirements](#direct-advertising-requirements) | Opt-out, record retention |
| [{{PROGRAM_NAME}} content compliance matrix](#program_name-content-compliance-matrix) | Which templates are compliant as-is |

---

## Illinois's prescriptive standard

Illinois specifies exactly what must and must not appear in gambling advertising. The core rule is **11 Ill. Adm. Code 1900.340** (sports wagering advertising and marketing). The casino master ad rule, **86 Ill. Adm. Code 3000.680**, carries the August 2025 uniform requirements, and the VGT ad rule (**11 Ill. Adm. Code 1800.1760**) and the sports ad rule (1900.340) incorporate 3000.680 by reference — which is how the requirements run consistently across all three verticals.

| Field | Value |
|---|---|
| **Primary regulation** | 11 Ill. Adm. Code 1900.340 (sports) |
| **Master ad rule** | 86 Ill. Adm. Code 3000.680 (casino) — incorporated by reference into 11 Ill. Adm. Code 1800.1760 (VGT) and 1900.340 (sports), as amended August 2025 |
| **Approach** | Prescriptive — specific content and conduct requirements apply automatically |
| **Enforcement** | IGB enforces proactively and publishes cease-and-desist letters |

### Content requirements

Every advertisement must:

| Requirement | Details | Regulatory basis |
|---|---|---|
| **Include problem gambling text** | The IDHS-prescribed compulsive-gambling text (set verbatim at 77 Ill. Adm. Code 2059.103), including the helpline | 1900.340(e)(2); 3000.680 |
| **State the 21+ requirement** | Ads must state patrons must be 21 years of age or older to wager | 1900.340(e)(3) |
| **Carry responsible-gaming messages** | Prominent responsible-gaming messaging | 1900.340; 3000.680 |
| **Not mislead** | No false or misleading content | 1900.340 |

The IDHS-prescribed text, required wherever the rules call for "problem gambling text as determined by IDHS," is:

> **"If you or someone you know has a gambling problem, crisis counseling and referral services can be accessed by calling 1-800-GAMBLER (1-800-426-2537)."**
> — 77 Ill. Adm. Code 2059.103

---

## What changed in August 2025

The August 1, 2025 uniform rules (JCAR-approved May 13, 2025; published in the Illinois Register) **extended consistent advertising requirements across casinos, video gaming, and sports wagering**. The notable additions:

| Change | Effect |
|---|---|
| **College advertising ban** | No advertising on college or university campuses, in college/university media, or at sports venues used primarily for college events |
| **No college depictions** | Advertising must not depict college students or college settings |
| **Affiliate compensation ban** | No performance-based affiliate compensation tied to the volume or outcome of wagers |
| **Opt-out** | Direct advertising must offer a method to opt out |
| **Record retention** | Operators must retain copies of all materials with a log of how and when each was published |
| **IDHS problem gambling text** | The 2059.103 text required across all three verticals |

These requirements are codified in the casino master ad rule **86 Ill. Adm. Code 3000.680**, which the VGT (1800.1760) and sports (1900.340) ad rules incorporate by reference.

---

## AGA Responsible Gaming Code

The **American Gaming Association (AGA)** Responsible Gaming Code of Conduct applies to Illinois operators that are AGA members. While not legally binding, it represents industry best practices:

| AGA principle | Illinois regulatory requirement | Status |
|---|---|---|
| Responsible advertising | 1900.340; 86 Adm. Code 3000; 11 Adm. Code 1800 | Illinois rules are more detailed than AGA principles |
| No targeting minors | 1900.340(e)(1) — must not target under-21 | Aligned |
| Helpline in ads | 1900.340(e)(2) — IDHS text (2059.103) including helpline | Aligned |
| No misleading claims | 1900.340 — no false or misleading content | Aligned |
| Self-exclusion | 1900.1730; 86 Adm. Code 3000.745–.790 — flag and bar self-excluded persons from marketing | Aligned |

---

## Channel-by-channel rules

Illinois's rules apply uniformly across channels for each vertical. The college-placement prohibitions are especially relevant to broadcast, digital, and in-venue placement decisions.

| Channel | Key requirements | Notes |
|---|---|---|
| **Broadcast (TV)** | IDHS problem gambling text + helpline; responsible-gaming message; no college-media placement | End card recommended for helpline |
| **Broadcast (radio)** | IDHS text + helpline spoken; no college-station placement | Verbal helpline reference |
| **Digital (display/video)** | IDHS text + helpline; no placement targeting under-21 or college audiences | Retain copies with publication log |
| **Digital (social media)** | IDHS text + helpline; opt-out for direct messages | Short-form may link to a landing page with full information |
| **Print** | IDHS text + helpline; no college-media placement | Full disclosures in print ads |
| **Direct marketing (email)** | IDHS text + helpline; opt-out method required | Retain copies; honor opt-outs |
| **Direct marketing (SMS/mail)** | IDHS text + helpline; opt-out method | No marketing to self-excluded persons |
| **In-venue (casino)** | Compulsive-gambling and treatment notices (230 ILCS 10/13.1); responsible-gaming messaging | Treatment + helpline information |
| **In-venue (VGT establishment)** | 21+ signage; problem-gambling sign (2059.103 text + Self-Exclusion Program access) at the gaming-area entrance and visible from any ATM (1800.1750); terminals in restricted area | No advertising aimed at under-21 |
| **Sponsorship** | Same content rules apply; no college-venue association | Must not target prohibited groups |

---

## Prohibited content

| Prohibition | Details | Source |
|---|---|---|
| **Targeting under-21** | Advertising must not be directed to individuals under 21 | 1900.340(e)(1) |
| **Appeal-to-minors content** | No imagery, symbols, cartoon/celebrity/entertainer figures, endorsements, or language designed to appeal to those under 21 | 1900.340(e)(4) |
| **Under-21 figures** | No featuring anyone who is, or appears to be, under 21 | 1900.340(e)(5) |
| **Under-21 media placement** | No placement in media (including social media) that appeals primarily to those under 21 | 1900.340(e)(6) |
| **Under-21 audiences** | No placement before any audience presumed to be majority under 21 | 1900.340(e)(7) |
| **College campuses** | No advertising on college or university campuses | 3000.680 (Aug 2025) |
| **College media** | No advertising in college/university newspapers, radio, or TV | 3000.680 (Aug 2025) |
| **College venues** | No advertising at sports venues used primarily for college events | 3000.680 (Aug 2025) |
| **Depicting college settings** | No depiction of college students or college settings | 3000.680 (Aug 2025) |
| **Performance-based affiliate pay** | No compensation tied to the volume or outcome of wagers | 1900.340(f); 3000.680 |
| **False or misleading content** | Prohibited | 1900.340 |

---

## Required disclosures

| Disclosure | Requirement | Where |
|---|---|---|
| **Problem gambling text** | IDHS-prescribed text (77 Ill. Adm. Code 2059.103), including the helpline | All advertising |
| **21+ statement** | Ads must state patrons must be 21 or older to wager | All advertising (1900.340(e)(3)) |
| **Responsible-gaming message** | Prominent responsible-gaming messaging | All advertising |
| **Opt-out method** | Clear method to opt out of future direct advertising | Direct ads |
| **Record retention** | Copies retained with a publication log | All advertising (internal) |
| **Casino venue notice** | Compulsive-gambling and treatment/helpline notices | Casino venues (230 ILCS 10/13.1) |
| **VGT signage** | 21-and-over signage, plus a problem-gambling sign (2059.103 text + Self-Exclusion Program access) at the gaming-area entrance and visible from any ATM | VGT establishments (1800.1750) |

---

## Direct advertising requirements

| Requirement | Details |
|---|---|
| **Opt-out method** | Each direct advertisement must provide a method by which an individual may opt out of receiving future advertisements |
| **Self-excluded persons** | Must flag self-excluded persons in all mailing/marketing/promotional lists and not send them marketing or promotional materials (1900.1730; 86 Adm. Code 3000.745–.790) |
| **Record retention** | Retain copies of all advertising and marketing materials with a log of when and how each was published, aired, displayed, or distributed (1900.340(c); 3000.680) |

---

## {{PROGRAM_NAME}} content compliance matrix

Assessment of which {{PROGRAM_NAME}} templates are Illinois-compliant as-is:

| Template | Illinois-compliant? | Adaptation needed |
|---|---|---|
| **Game guides (HTP cards)** | Yes | No changes — educational content with no advertising claims |
| **Core messages** | Yes, with adaptation | Add 1-800-GAMBLER and IDHS problem gambling text where helpline tokens are used |
| **Myth-busting** | Yes | No changes — factual corrections are compliant |
| **CTAs** | Review needed | Ensure no CTAs target under-21 or imply guaranteed success |
| **Posters** | Yes, with adaptation | Replace helpline with 1-800-GAMBLER; add IDHS text; no college placement |
| **Venue signage** | Yes, with adaptation | Include compulsive-gambling and treatment notices (230 ILCS 10/13.1) |
| **Email templates** | Yes, with adaptation | Add 1-800-GAMBLER and IDHS text; opt-out for direct ads |
| **Social cards** | Review needed | Avoid college audiences; link to landing page with full disclosures |
| **RG page** | Yes, with adaptation | Add 1-800-GAMBLER, SEP details, "Are You Really Winning?" resources |
| **Staff FAQ** | Yes, with adaptation | Add SEP Q&A and 1-800-GAMBLER |
| **Conversation scripts** | Yes, with adaptation | Add 1-800-GAMBLER in referral scripts; add SEP enrollment |

### On-brand disclosure examples

**Sports wagering promotion (email):**
> **This week's odds boost: Bears +120 vs. Packers**
>
> Place your bet with [Operator Name].
>
> Odds may change. Must be 21+. Illinois only. T&Cs apply at [website].
> Gambling problem? Call 1-800-GAMBLER (24/7, free, confidential).
> Text ILGAMB to 833234. [Unsubscribe link]

**Sports wagering promotion (social media — short form):**
> Bears +120 this Sunday. Bet now at [Operator].
> 21+ | Gambling problem? 1-800-GAMBLER | Full terms at [link]

**Casino venue signage ({{PROGRAM_NAME}} approach):**
> **Play on your terms.** Set your limits. Know the odds.
> Need a break? The Illinois Self-Exclusion Program covers casinos and sportsbooks statewide.
> Free, confidential support 24/7: **1-800-GAMBLER** | Text ILGAMB to 833234

---

## Comparison with other US jurisdictions

| Rule | Illinois (1900.340 + Aug 2025) | Ohio (OAC 3775-16-08) | Nevada (Reg 5.011(1)(d)) |
|---|---|---|---|
| **Approach** | Prescriptive | Prescriptive | Principles-based |
| **Problem gambling text** | Required (IDHS-determined) | Required | Not required in ads (signage only) |
| **Helpline in ads** | Required | Required | Not required in ads |
| **College advertising** | Banned (campuses, media, venues, depictions) | Not specifically banned | Not specified |
| **Affiliate compensation** | No performance-based pay | Not specified | Not specified |
| **Anti-targeting** | Under-21 | Under-21, problem gamblers, vulnerable | Under-21 (NRS 463.0129) |
| **Direct ad opt-out** | Required | 15-day window | Not specified |
| **Enforcement** | Proactive monitoring; cease-and-desist letters | Proactive monitoring | Complaint-driven |

---

*Cross-references: [Illinois compliance module](README.md) | [Ohio advertising rules](../ohio/advertising-rules.md) | [Nevada advertising rules](../nevada/advertising-rules.md) | [Messaging Framework — Warning Statement Standards](../../../brand-book/05-messaging-framework.md#warning-statement-standards)*
