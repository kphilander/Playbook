 ...
 *   brand.colorsHex()                // { navy: '#1B2838', teal: '#10B981', ... }
 *   brand.colorsRaw()                // { navy: '1B2838', teal: '10B981', ... }
 *   brand.fontFamilies()             // { heading: "'Inter', system-ui, ...", ... }
 *   brand.helpline()                 // { number: "1-800-522-4700", ... }
 *   brand.brandTokens()              // { '{{PROGRAM_NAME}}': 'Playbook', ... }
 */
...
export function createBrandConfig(data) {
   ...
   * e.g. { primary: '#1B2838', secondary: '#10B981', ... }
   */
  function paletteHex() {
    return { ...palette };
  }
   ...
   * Sub-region ages live under combined keys in _brand.yml
   * (e.g. legal.minimum_gambling_age.canada-british-columbia),
   * so the combined key wins over the jurisdiction-wide value.
   * @param {string} [jurisdiction='default']
   * @param {string} [subRegion]
   */
  function minAge(jurisdiction = 'default', subRegion = null) {
    const ages = data.legal?.minimum_gambling_age || {};
    return (subRegion != null ? ages[`${jurisdiction}-${subRegion}`] : undefined)
      ?? ages[jurisdiction]
      ?? ages.default
      ?? 18;
  }
   ...
   */
  function culturalProfile(country = null) {
    // If a country is specified, return that country's profile;
    // otherwise return the default brand-wide profile.
    const source = country
      ? (data.country_cultural_profiles?.[country] || data.cultural_profile || {})
      : (data.cultural_profile || {});
    return {
      voice: source.voice || 'peer',
      framing: source.framing || 'individual',
      humor: source.humor || 'irreverent',
      directness: source.directness || 'blunt',
      comfort: source.comfort || 'open',
    };
  }

  /**
   * Get currency configuration for a jurisdiction.
   */
  function currency(jurisdiction = null) {
    const all = data.currency || {};
    return all[jurisdiction] || all.default || { symbol: '$', code: 'USD', position: 'before' };
  }
  ...
  function brandTokens(jurisdiction = 'united-states', subRegion = 'national') {
    ...
    const age = String(minAge(jurisdiction, subRegion));
    const ageDisclaimerTemplate = data.legal?.disclaimers?.age_by_jurisdiction?.[jurisdiction]
      || data.legal?.disclaimers?.age
      || 'You must be {{MIN_AGE}}+ to gamble.';
    const ageDisclaimer = ageDisclaimerTemplate
      .replaceAll('{{MIN_AGE}}', age);
    const generalDisclaimer = data.legal?.disclaimers?.general || 'Gambling involves risk. Play on your terms.';
    const curr = currency(jurisdiction);
    return {
      '{{PROGRAM_NAME}}': data.meta?.program_name || 'Playbook',
      '{{PROGRAM_SHORT_NAME}}': data.meta?.short_name || data.meta?.program_name || 'Playbook',
      ...
      '{{HELPLINE_HOURS}}': hl.hours || '',
      '{{HELPLINE_WEBSITE}}': hl.website || '',
      '{{LIMITS_URL}}': data.integration?.limits_url || '',
      '{{SELF_EXCLUSION_URL}}': data.integration?.self_exclusion_url || '',
      '{{EMAIL_PREFERENCES_URL}}': data.integration?.preferences_url || '',
      '{{MIN_AGE}}': age,
      ...
      // Currency tokens (resolved per jurisdiction)
      '{{CURRENCY_SYMBOL}}': curr.symbol || '$',
      '{{CURRENCY_CODE}}': curr.code || 'USD',
