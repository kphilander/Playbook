...
import { surfaceTokens, readableOn } from '../hub/configurator/js/color-utils.js';

const brand = loadBrand();
const OUTPUT = join(brand.ROOT, 'collateral', 'render', 'brand-inject.css');
const LAYOUT_SOURCE = join(brand.ROOT, 'collateral', 'render', 'layout-system.css');
...
// Gradients — used by 60+ templates for bars, backgrounds, and overlays
lines.push('  /* ─── Gradients ────────────────────── */');
lines.push(`  --pb-gradient-bar: linear-gradient(90deg, ${palette.accent || '#FF6B35'}, ${palette.secondary || '#10B981'});`);
lines.push(`  --pb-gradient-bg: linear-gradient(180deg, ${palette.primary || '#1B2838'}, ${palette.primary_dark || '#0F1923'});`);
lines.push('');

// Surface polish — ink + texture that adapt to the surface luminance, so the
// gradient + grid card treatment (16a/16b/17a/18a) holds on any brand palette.
const surf = surfaceTokens(palette.primary || '#1B2838', { grid: true });
const brandInk = readableOn(palette.primary || '#1B2838', '#FFFFFF', 4.5);
lines.push('  /* ─── Surface Polish ───────────────── */');
lines.push(`  --pb-shading: 1;`);
lines.push(`  --pb-grid: 1;`);
lines.push(`  --pb-surface-ink: ${surf.ink};`);
lines.push(`  --pb-surface-ink-muted: ${surf.inkMuted};`);
lines.push(`  --pb-texture-line: ${surf.textureLine};`);
lines.push(`  --pb-surface-track: ${surf.track};`);
lines.push(`  --pb-surface-track-border: ${surf.trackBorder};`);
lines.push(`  --pb-brand-ink: ${brandInk};`);
lines.push('');
...
// Maintained separately so brand token generation never erases the shared
// responsive, format, safe-zone, and production-export rules.
if (existsSync(LAYOUT_SOURCE)) {
  lines.push('/* ─── Shared collateral layout system ─────────────── */');
  lines.push(readFileSync(LAYOUT_SOURCE, 'utf-8').trim());
  lines.push('');
}
