...
console.log('\nMinimum gambling age:');
const ages = brand.legal?.minimum_gambling_age || {};
if (ages.default == null) {
  warn('legal.minimum_gambling_age.default not set (code falls back to 18)');
} else {
  ok(`default: ${ages.default}`);
}

// Sub-region ages use combined keys (e.g. canada-british-columbia: 19).
// Verify each one resolves through minAge(jurisdiction, subRegion).
let subRegionAges = 0;
let subRegionAgeErrors = 0;
for (const [jurisdiction, juris] of Object.entries(helplines)) {
  if (jurisdiction === 'default' || typeof juris !== 'object' || juris === null) continue;
  for (const [subRegion, entry] of Object.entries(juris)) {
    if (typeof entry !== 'object' || entry === null) continue;
    const combinedKey = `${jurisdiction}-${subRegion}`;
    if (ages[combinedKey] == null) continue;
    subRegionAges++;
    const resolved = brand.minAge(jurisdiction, subRegion);
    if (resolved !== ages[combinedKey]) {
      error(`minAge("${jurisdiction}", "${subRegion}") = ${resolved} — expected ${ages[combinedKey]} from key "${combinedKey}"`);
      subRegionAgeErrors++;
    }
  }
}
if (subRegionAges > 0 && subRegionAgeErrors === 0) {
  ok(`All ${subRegionAges} sub-region age keys resolve through minAge()`);
}
