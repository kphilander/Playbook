...
function section(d){
  const treatments=treatmentsFor(d),isNew=d.round===4;
  ...
  <header class="concept-intro"><p class="eyebrow">${isNew?'New direction':'Earlier concept'} / ${number}</p><h2>${escapeHTML(d.title.slice(5))}</h2><p>${escapeHTML(d.rationale)}</p></header>
...
writeFileSync(new URL('index.html',root),`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Four new creative directions · Playbook</title><link rel="stylesheet" href="review.css?v=4"></head><body>
