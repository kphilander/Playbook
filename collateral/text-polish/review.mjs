const key='playbook-text-polish-preferences-v1';
...
function show(saved){const choices=read();for(const section of document.querySelectorAll('[data-id]')){const value=choices[section.dataset.id];for(const b of section.querySelectorAll('[data-choice]'))b.setAttribute('aria-pressed',String(b.dataset.choice===value));section.querySelector('output').textContent=['keep','refine'].includes(value)?(saved===section?'Preference saved':'Saved preference'):'';}}
