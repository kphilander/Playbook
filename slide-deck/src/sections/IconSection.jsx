    ...
    <div className="px-6 sm:px-10 lg:px-16 py-24 lg:py-32 max-w-6xl mx-auto">
      ...
      <ScrollReveal className="mb-8">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-n700" />
            <input
              type="text"
              value={search}
      ...
      <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 gap-3">
        {filtered.map((icon) => (
          ...
          >
            <img
              src={`${BASE}assets/icons/icon-${icon.name}.svg`}
              alt={icon.name}
      ...
      {filtered.length === 0 && (
        <p className="text-center text-n500 font-body text-sm py-12">No icons match your search.</p>
      )}
